...
if overwriteFlag
	...
	for subjectNumber = 1:nSubjects % and for each subject...
        ...
        %betaCorrespondence = '/hpc/banco/dagens.t/Subj1/Beta_maps_25x25_glm/[[betaIdentifier]]'% ADDED
		...
		searchlightOptions.nSessions = 1; %size(betas, 1);
		searchlightOptions.nConditions = 25; %size(betas, 2);
        
        
        
		readFile = replaceWildcards(userOptions.betaPath, '[[subjectName]]', subject, '[[betaIdentifier]]', 'Beta01.img');
...
%		subjectMetadataStruct = spawnSPMStruct;
		...
		for maskNumber = 1:nMasks % For each mask...
			...
			for modelNumber = 1:numel(models)
				...
				modelName = underscoresToSpaces(models(modelNumber).name);
				...
				if isfield(userOptions, 'structuralsPath')
                    ...
                    disp(userOptions.structuralsPath)
%_seg_sn            
