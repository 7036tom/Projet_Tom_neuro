...
if overwriteFlag
    ...
    % A COMPLETER : (important, nombre de conditions. Nombre de sessions toujours ? 1 dans le cadre de glm denoise)
	nConditions = 25;%size(betas, 2);
	nSessions = 1;%size(betas, 1);
	...
	for subject = 1:nSubjects % For each subject
		...
		for session = 1:nSessions % For each session...
            ...
            % A COMPLETER (important : chemin vers le dossier output_glm_denoise)
            readPath = '/hpc/banco/dagens.t/Subj1/A_dir/glmdenoiseEstimates.mat';
            load(readPath, 'results');
			...
			for condition = 1:nConditions % and each condition...
                ...
                brainMatrix = results.modelmd{2}(:,:,:,condition) ;
